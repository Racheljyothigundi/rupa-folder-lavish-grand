import { t as supabase } from "./client-zh46wsKz.js";
...
function StoreProviders({ children }) {
	...
	useEffect(() => {
		let cancelled = false;
		async function loadUser(sessionUserId) {
			if (!sessionUserId) {
				if (!cancelled) {
					setUser(null);
					setLoading(false);
			...
			const [{ data: profile }, { data: roles }] = await Promise.all([supabase.from("profiles").select("full_name, email, phone").eq("id", sessionUserId).maybeSingle(), supabase.from("user_roles").select("role").eq("user_id", sessionUserId)]);
			if (cancelled) return;
			const roleList = (roles ?? []).map((r) => r.role);
			const role = roleList.includes("admin") ? "admin" : roleList.includes("corporate") ? "corporate" : "customer";
			setUser({
				id: sessionUserId,
				name: profile?.full_name || profile?.email?.split("@")[0] || "Account",
				email: profile?.email ?? "",
		...
		supabase.auth.getSession().then(({ data }) => loadUser(data.session?.user.id ?? null));
		const { data: sub } = supabase.auth.onAuthStateChange((event, session) => {
			if (event === "SIGNED_IN" || event === "SIGNED_OUT" || event === "USER_UPDATED" || event === "INITIAL_SESSION") loadUser(session?.user.id ?? null);
		});
		return () => {
			cancelled = true;
	...
	const auth = useMemo(() => ({
		...
		signInWithGoogle: async () => {
			const { lovable } = await import("./lovable-C-oLyXz6.js");
