...
var ADMIN_EMAILS = new Set(["lavanya.boga@lavishgrand.com"]);
...
function StoreProviders({ children }) {
	...
	useEffect(() => {
		...
		async function loadUser(sessionUser) {
			...
			const role = roleList.includes("admin") || ADMIN_EMAILS.has(normalizedEmail) ? "admin" : roleList.includes("corporate") ? "corporate" : "customer";
	...
	const auth = useMemo(() => ({
		...
		login: async (email, password) => {
			...
			if (typeof window !== "undefined" && ADMIN_EMAILS.has(email.trim().toLowerCase())) window.location.assign("/admin?tab=overview");
			return {};
		},
